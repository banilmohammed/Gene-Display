...
class streamlit:
    ...
    # Class Parameters
    dataframes = [ 'GSE460_series_matrix'] #List of dataframes in database.
    inputGenes = [] # List of gene names that the user has inputted.
    datasets = [] # List of datasets that the user has chosen.
    geneDataframes = dict() # Dictionary with keys as the dataset names, and values as the dataset specified by the user's inputs of the genes.
    userDataframes = dict() # Dictionary with keys as the dataset names, and values as the dataset specified by the user's inputs of the replicates.
    ...
    def createSidebar():
        ...
        st.sidebar.markdown('Select Two Datasets to Compare') # set markdown title telling user to choose datasets
        streamlit.datasets = st.sidebar.multiselect('Select Datasets', streamlit.dataframes) # create multiselect option of the datasets
        st.sidebar.markdown('Input Gene Names (separate by commas if more than one gene)') # set markdown title telling user to input genes
        userInput = st.sidebar.text_input("Gene names: ") # create a text box for user to input genes
        newString = userInput.replace(" ", "") # delete the whitespace within the user's input
        streamlit.inputGenes = newString.split(',') #split the string by commas
        st.sidebar.markdown('Choose Replicates to be Graphed') # set markdown title telling user to choose the replicates
        streamlit.selectReplicates = st.sidebar.radio('Replicates', ('replicate1', 'replicate2', 'replicate3', 'average')) # create a selection of the replicate options to choose from
        st.sidebar.markdown('Graphing') # set markdown title of the graphing button
        streamlit.graphButton = st.sidebar.checkbox("Click to Graph") # create a checkbox the user can click on to graph
    ...
    def navBar():
        ...
        if streamlit.graphButton: # check if graphbutton is selected
            ...
            streamlit.selectNavigation = st.sidebar.radio('Go to', ('Comparison', 'Individual', 'Dataframe')) #create a selection of the different pages
    ...
    def specifyDataframes():
        ...
        Create dictionary of the datasets specified by the user's selected options for plotting.
        '''
        for i in streamlit.geneDataframes: #iterate over the specified dataframes with the genes that the user selected
            df = streamlit.geneDataframes[i] # get the dataframe
            if streamlit.selectReplicates != 'average': # check if the selected replicate is not average
                colDrop = [] # create list of the columns to be dropped
                for column in df.columns: #iterate over all the columns of the dataframe
                    ...
                    if streamlit.selectReplicates not in column and column != 'Gene Description' and column != 'Gene Name' and column != 'HG ID' and column != 'RefSeq':
                        colDrop.append(column) # append the column to the list of colDrop
                cleanedDF = df.drop(columns = colDrop) #drop all the columns in the list colDrop
                cleanedDF = cleanedDF.set_index('Gene Name') #set the index to Gene name
                ...
                geneID = df['HG ID'].tolist() # set variable gene ID to the values within the column HD ID
                refSeq = df['RefSeq'].tolist()# set variable refSeq to the values within the column RefSeq
                geneDescription = df['Gene Description'].tolist()# set variable geneDescription to the values within the column Gene Description
                geneName = df['Gene Name'].tolist()# set variable geneName to the values within the column Gene Name
                df = df.drop(columns = ['Gene Description', 'Gene Name', 'RefSeq', 'HG ID']) #drop columns Gene Description, Gene Name, RefSeq, and HG ID
                for column in df.columns: #iterate over all the columns
                    newString = column[0:column.index('_')+1] # create a string with just the time
                    finalString = newString + 'average' #then concatenate '_average' to the string that has the time
                ...
                df = df.groupby('index', sort=False).mean().transpose() #group the dataframe by the column 'index' and find the mean and then transpose
                ...
                cleanedDF = df.set_index('Gene Name') #set the index of the dataframe to the column Gene Name
    ...
    def createDataframes():
        ...
        Print out the dataframes specified to the users inputs.
        '''
        if streamlit.selectNavigation == 'Dataframe': #check if the user chose dataframe in the navbar
            st.header('Dataframes') # set the header name to 'Dataframes'
            for df in streamlit.userDataframes: #iterate over the filtered out dataframes
        ...
        if streamlit.graphButton and streamlit.selectNavigation == 'Comparison':
            ...
            if len(dataframes) > 1: #if the length of datframes is list is more than one
                ...
                for data in dataframes:
                    fig, ax = plt.subplots()  #plot subplots
                    for column in set(data.T.columns.tolist()):#iterate over the transposed dataframe columns
                        data.T.plot(kind = 'line', y = column, ax=ax, rot=45, alpha = 0.5)  #plot each column into the graph of the transposed DataFrame
                    # set the title name, x-axis name, y-axis name, and put the legend outside
                    plt.title('Comparing Datasets')
                    plt.xlabel('Time')
                    plt.ylabel('Decay')
